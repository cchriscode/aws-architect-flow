...
상태 페이지 관리와 인시던트 관리를 하나의 플랫폼에서 통합한 도구예요. 내부 인시던트 대응과 외부 커뮤니케이션을 동시에 처리할 수 있어요.

```
FireHydrant 특징:

  🌐 상태 페이지 통합 (강점)
    • 인시던트 선언 시 상태 페이지 자동 업데이트
    • 컴포넌트별 상태 관리 (API, Dashboard, Auth 등)
    • 고객 구독 알림 자동 발송
    • 내부 인시던트 ↔ 외부 상태 페이지 연동

  🎭 역할 기반 인시던트 관리
    • Incident Commander, Communications Lead 등 역할 자동 배정
    • 역할별 체크리스트 자동 제공
    • Runbook과 역할 매핑

  📋 레트로스펙티브 자동화
    • 타임라인 자동 생성 (Slack 메시지 + 상태 변경 기반)
    • 영향 분석 자동 집계 (영향 시간, 영향 고객 수)
    • 액션 아이템 → Jira/Shortcut 자동 동기화

  🔗 서비스 카탈로그
    • 서비스 소유권, 의존성 맵핑
    • 인시던트 발생 시 관련 서비스 자동 식별
    • 변경 이벤트 추적 (배포 → 인시던트 상관관계)
```

#### 2024-2025 인시던트 관리 플랫폼 트렌드

기존 On-call 중심 도구(PagerDuty, OpsGenie)와 차세대 인시던트 관리 플랫폼(Incident.io, Rootly, FireHydrant)은 근본적으로 다른 문제를 풀어요.

```
기존 도구 (PagerDuty, OpsGenie):
  "누가 알림을 받을 것인가?" → On-call, 에스컬레이션, 알림 라우팅

차세대 플랫폼 (Incident.io, Rootly, FireHydrant):
  "인시던트를 어떻게 효과적으로 관리할 것인가?" → 협업, 자동화, 학습

  2024-2025 핵심 트렌드:
  ┌─────────────────────────────────────────────────────────────┐
  │ 1. AI 기반 자동화                                          │
  │    • Incident.io: AI가 유사 과거 인시던트 자동 제안        │
  │    • Rootly: AI 포스트모템 초안 자동 생성                  │
  │    • FireHydrant: AI 기반 영향 범위 자동 추정              │
  │                                                             │
  │ 2. 플랫폼 엔지니어링 통합                                  │
  │    • 서비스 카탈로그와 인시던트 자동 연결                   │
  │    • Backstage / Port / OpsLevel 연동                      │
  │    • 배포 이벤트 → 인시던트 상관관계 자동 분석             │
  │                                                             │
  │ 3. 조합형 아키텍처                                         │
  │    • PagerDuty (알림) + Incident.io (관리) 조합이 주류     │
  │    • On-call 도구와 인시던트 관리 도구는 별도 영역         │
  │    • API 기반 통합으로 Best-of-breed 구성                  │
  └─────────────────────────────────────────────────────────────┘
```
...
```
┌──────────────┬────────────┬─────────────┬──────────────┬────────────┬──────────────┐
│   기능       │ PagerDuty  │ OpsGenie    │ Incident.io  │ Rootly     │ FireHydrant  │
├──────────────┼────────────┼─────────────┼──────────────┼────────────┼──────────────┤
│ On-call      │ ⭐⭐⭐⭐⭐ │ ⭐⭐⭐⭐    │ ⭐⭐⭐       │ ⭐⭐⭐     │ ⭐⭐⭐       │
│ 에스컬레이션 │ ⭐⭐⭐⭐⭐ │ ⭐⭐⭐⭐    │ ⭐⭐⭐       │ ⭐⭐⭐     │ ⭐⭐⭐       │
│ Slack 통합   │ ⭐⭐⭐     │ ⭐⭐⭐      │ ⭐⭐⭐⭐⭐   │ ⭐⭐⭐⭐⭐ │ ⭐⭐⭐⭐     │
...
│ 분석/리포트  │ ⭐⭐⭐⭐⭐ │ ⭐⭐⭐      │ ⭐⭐⭐⭐     │ ⭐⭐⭐⭐   │ ⭐⭐⭐⭐     │
│ 가격         │ $$$        │ $$          │ $$$          │ $$         │ $$$          │
│ 적합한 팀    │ 대기업     │ Atlassian팀 │ Slack 중심   │ 스타트업   │ 고객 대면팀  │
└──────────────┴────────────┴─────────────┴──────────────┴────────────┴──────────────┘

추천 조합 (2024-2025):
  소규모 팀 (5-15명):  Rootly (무료 플랜) 또는 OpsGenie
  중규모 팀 (15-50명): PagerDuty (On-call) + Incident.io (관리)
  대규모 팀 (50명+):   PagerDuty + Incident.io + FireHydrant (상태 페이지)
  고객 대면 서비스:     FireHydrant (상태 페이지 통합이 핵심)

선택 가이드:
  • On-call/에스컬레이션이 최우선 → PagerDuty
  • Atlassian 생태계 사용 중 → OpsGenie
  • Slack에서 모든 것을 처리하고 싶음 → Incident.io
  • 워크플로 자동화가 핵심 → Rootly
  • 외부 상태 페이지 + 인시던트 통합 → FireHydrant
```
